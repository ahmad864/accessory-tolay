...
export default function AdminProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("products").select("id,name,price,image,category,low_stock").order("name");
    ...
    <div className="p-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 border p-4 rounded bg-gray-50">
          ...
          <input type="text" name="category" placeholder="الفئة" value={formData.category} onChange={handleChange} required className="border p-2 w-full mb-2"/>
          <label className="flex items-center mb-2">
            <input type="checkbox" name="low_stock" checked={formData.low_stock} onChange={handleChange} className="mr-2"/>
            الكمية قليلة
          </label>
          <input type="file" name="imageFile" accept="image/png" onChange={handleChange}/>
